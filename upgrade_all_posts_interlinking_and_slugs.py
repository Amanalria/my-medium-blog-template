#!/usr/bin/env python3
"""
Comprehensive Upgrade Script for HiveCloud.in:
1. 3-Word Slug for all 11 posts.
2. Under 60-character Title for all 11 posts.
3. Natural Semantic Distribution of all 40 Keywords across prose (No list box).
4. Universal Interlinking on EVERY post:
   - 1 Link to Homepage (https://hivecloud.in/)
   - 2 Internal links to other relevant posts on hivecloud.in
   - 1 Authoritative External Source link
5. Over 1600 words per article.
6. Supabase Synchronization, Sitemap update, and Git deployment.
"""
...
ADMIN_JS = os.path.join(REPO_DIR, "admin.js")
...
# ════════════════════════════════════════════════════════════════════════════════
ART1_TITLE = "Agentic AI Japan: Autonomous Enterprise & METI Guide"  # 53 chars (<60)
ART1_SLUG = "agentic-ai-japan"  # 3 words
ART1_SUBTITLE = "How Japan solves demographic labor challenges with METI GENIAC subsidies, Sakana AI, NTT Tsuzumi, and sovereign agent swarms."
...
<p>This massive compute push aligns closely with breakthroughs in <a href="/ai-reasoning-compute" class="text-emerald-600 font-semibold underline">test-time compute and reasoning models</a>, where thinking time during inference delivers dramatic accuracy gains for autonomous systems.</p>
...
# ════════════════════════════════════════════════════════════════════════════════
ART2_TITLE = "Claude Code Anthropic: Latest News & Agentic CLI Guide"  # 54 chars (<60)
ART2_SLUG = "claude-code-anthropic"  # 3 words
ART2_SUBTITLE = "Anthropic's terminal agent transforms software development with Claude 3.7 Sonnet hybrid reasoning, autonomous bash execution, and MCP tools."
...
<p>This terminal-first execution model perfectly complements multi-agent development pipelines explained in our guide to <a href="/agentic-ai-coding" class="text-emerald-600 font-semibold underline">agentic AI coding workflows</a>.</p>
...
<ol>
 ...
 <li><strong>Model Context Protocol (MCP) Integration:</strong> Exploring <strong>using model context protocol mcp servers inside claude code</strong> enables seamless connection with external database servers, API documentation endpoints, and custom developer tools, as detailed in our guide on <a href="/multi-agent-orchestration" class="text-emerald-600 font-semibold underline">multi-agent orchestration with MCP</a>.</li>
...
# ════════════════════════════════════════════════════════════════════════════════
ART3_TITLE = "Google Student Plan: Free Perks, Cloud & Gemini Guide"  # 53 chars (<60)
ART3_SLUG = "google-student-plan"  # 3 words
ART3_SUBTITLE = "How college and university students access free Google Workspace, Gemini Advanced, Google Cloud credits, and 2TB storage."
...
def main():
    print("🚀 Running Universal 3-Word Slug, <=60 Char Title & Universal Interlinking Update...")

    # Load existing articles
    with open(MAIN_JSON, "r", encoding="utf-8") as f:
        articles = json.load(f)

    # Clean the 3 target articles
    c1_html, c1_t, c1_s = clean(ART1_HTML, ART1_TITLE, ART1_SUBTITLE)
    c2_html, c2_t, c2_s = clean(ART2_HTML, ART2_TITLE, ART2_SUBTITLE)
    c3_html, c3_t, c3_s = clean(ART3_HTML, ART3_TITLE, ART3_SUBTITLE)

    # Standardized metadata mapping for all 11 articles
    # Every post: exactly 3-word slug, <= 60 character title, 1 homepage link, 2 internal links, 1 external link
    all_upgraded_articles = [
        {
            "id": 9,
            "title": c1_t,
            "slug": ART1_SLUG,
    ...
    # Process remaining 8 articles to enforce 3-word slugs, <= 60 char titles, and verified interlinking
    remaining_meta = {
        1: {
            "slug": "agentic-ai-coding",  # 3 words
            "title": "Agentic AI Coding: Multi-Agent Workflows Guide 2026",  # 51 chars
            "int1": "/claude-code-anthropic",
            "int2": "/autonomous-ai-production",
            "ext_url": "https://www.swebench.com/",
            "ext_txt": "SWE-bench Official Verified Benchmarks"
        },
        2: {
            "slug": "ai-reasoning-compute",  # 3 words
            "title": "AI Reasoning Leap: Test-Time Compute Architecture",  # 49 chars
            "int1": "/agentic-ai-japan",
            "int2": "/context-engineering-memory",
            "ext_url": "https://arxiv.org/abs/2410.02122",
            "ext_txt": "Process Reward Models Research Paper (arXiv)"
        },
        3: {
            "slug": "autonomous-ai-production",  # 3 words
            "title": "Autonomous AI Production: Enterprise Architecture Guide",  # 55 chars
            "int1": "/agentic-ai-coding",
            "int2": "/multi-agent-orchestration",
            "ext_url": "https://opentelemetry.io/",
            "ext_txt": "OpenTelemetry Distributed Tracing Specification"
        },
        4: {
            "slug": "multi-agent-orchestration",  # 3 words
            "title": "Multi-Agent Orchestration: Complete MCP System Guide",  # 53 chars
            "int1": "/claude-code-anthropic",
            "int2": "/autonomous-ai-production",
            "ext_url": "https://modelcontextprotocol.io/",
            "ext_txt": "Model Context Protocol (MCP) Official Standard"
        },
        5: {
            "slug": "context-engineering-memory",  # 3 words
            "title": "Context Engineering: How Dynamic AI Memory Powers Apps",  # 54 chars
            "int1": "/ai-reasoning-compute",
            "int2": "/agentic-ai-coding",
            "ext_url": "https://redis.io/docs/latest/develop/data-types/vector-search/",
            "ext_txt": "Redis Vector Storage & Real-Time Memory Docs"
        },
        6: {
            "slug": "agentic-ai-news",  # 3 words
            "title": "Agentic AI News: Enterprise Shift to Parallel Swarms",  # 52 chars
            "int1": "/agentic-ai-japan",
        ...
        7: {
            "slug": "ai-agents-news",  # 3 words
            "title": "AI Agents News: LangGraph vs CrewAI in Production",  # 49 chars
            "int1": "/multi-agent-orchestration",
            "int2": "/autonomous-ai-production",
            "ext_url": "https://langchain-ai.github.io/langgraph/",
            "ext_txt": "LangGraph Stateful Orchestration Framework"
        },
        8: {
            "slug": "latest-agentic-news",  # 3 words
            "title": "Latest Agentic News: Sovereign Enterprise Agent Hubs",  # 52 chars
            "int1": "/agentic-ai-japan",
    ...
    # Find existing posts by ID 1..8 and update them
    for old_art in articles:
        aid = old_art.get("id")
        if aid in remaining_meta:
            meta = remaining_meta[aid]
            content = old_art.get("content", "")
            
            # Ensure 1 homepage link
            if "https://hivecloud.in/" not in content and 'href="/"' not in content:
                content = content.replace("<p>", f'<p>Explore the latest architectural deep dives on the <a href="https://hivecloud.in/" class="text-emerald-600 font-semibold underline">HiveCloud Engineering Hub</a>. ', 1)
            
            # Ensure 2 internal links
            if meta["int1"] not in content:
                content += f'\n<p class="mt-4 text-xs theme-muted">Related reading: explore our comprehensive guide on <a href="{meta["int1"]}" class="text-emerald-600 font-semibold underline">{meta["int1"].replace("/", "").replace("-", " ").title()}</a> and <a href="{meta["int2"]}" class="text-emerald-600 font-semibold underline">{meta["int2"].replace("/", "").replace("-", " ").title()}</a>.</p>'
            
            # Ensure 1 external link
            if meta["ext_url"] not in content:
                content += f'\n<p class="text-xs theme-muted">Authoritative reference: consult the <a href="{meta["ext_url"]}" target="_blank" rel="noopener noreferrer" class="text-emerald-600 font-semibold underline">{meta["ext_txt"]}</a>.</p>'
            
            cleaned_c = humanizer.clean_ai_patterns(content)
            all_upgraded_articles.append({
                "id": aid,
                "title": meta["title"],
                "slug": meta["slug"],
                "subtitle": old_art.get("subtitle", ""),
                "category": old_art.get("category", "Artificial Intelligence"),
                "tags": old_art.get("tags", ""),
                "author": old_art.get("author", "Aman Alria"),
                "readTime": old_art.get("readTime", "8 min read"),
                "content": cleaned_c,
                "wordCount": len(re.sub(r'<[^>]+>', ' ', cleaned_c).split())
            })

    # Validate all 11 articles
    print("\n--- Validating All 11 Posts ---")
    for a in all_upgraded_articles:
        slug_words = len(a["slug"].split("-"))
        t_len = len(a["title"])
        w_cnt = a["wordCount"]
        print(f"ID: {a['id']:2d} | Slug ({slug_words}w): /{a['slug']:<26} | Title ({t_len:2d}c): {a['title']} | Words: {w_cnt}")
        assert slug_words == 3, f"Slug /{a['slug']} does not have exactly 3 words!"
        assert t_len <= 60, f"Title '{a['title']}' exceeds 60 chars ({t_len})!"
        assert w_cnt >= 1500, f"Post /{a['slug']} has only {w_cnt} words!"

    # 1. Update articles_data.json
    with open(MAIN_JSON, "w", encoding="utf-8") as f:
        json.dump(all_upgraded_articles, f, indent=2)
    print(f"\n✅ Updated {MAIN_JSON} with 11 standardized articles.")
    ...
    with open(PRELOAD_JS, "w", encoding="utf-8") as f:
        f.write(f"window.__PRELOADED_ARTICLES__ = {json.dumps(all_upgraded_articles, indent=2)};\n")
...
"""
    ...
    # 4. Sync All 11 Posts to Supabase
    headers = {
        "apikey": ANON_KEY,
    ...
    # 5. Update index.html & admin.js links to new 3-word slugs
    with open(INDEX_HTML, "r", encoding="utf-8") as f:
        idx = f.read()
    idx = idx.replace("agentic-ai-coding-guide-2026", "agentic-ai-coding")
    with open(INDEX_HTML, "w", encoding="utf-8") as f:
        f.write(idx)

    with open(ADMIN_JS, "r", encoding="utf-8") as f:
        adm = f.read()
    adm = adm.replace("agentic-ai-coding-guide-2026", "agentic-ai-coding")
    adm = adm.replace("ai-reasoning-test-time-compute", "ai-reasoning-compute")
    adm = adm.replace("autonomous-ai-agents-production-guide", "autonomous-ai-production")
    adm = adm.replace("multi-agent-orchestration-mcp-guide", "multi-agent-orchestration")
    adm = adm.replace("context-engineering-dynamic-memory-guide", "context-engineering-memory")
    with open(ADMIN_JS, "w", encoding="utf-8") as f:
        f.write(adm)
    print(f"✅ Updated references in index.html & admin.js.")

    # 6. Update wp-poster data
    wp_arts = []
    for a in all_upgraded_articles:
        wp_arts.append({
            "id": f"art_{a['slug'].replace('-', '_')}",
            "title": a["title"],
            "content": a["content"],
            "slug": a["slug"],
            "excerpt": a["subtitle"],
    ...
    with open("/root/wp-poster/data/articles.json", "w", encoding="utf-8") as f:
        json.dump(wp_arts, f, indent=2)
    print("✅ Synced to /root/wp-poster/data/articles.json")

    # 7. Git Commit and Push
    print("\n📦 Committing and Pushing to GitHub (https://hivecloud.in)...")
    try:
        subprocess.run(["git", "add", "."], cwd=REPO_DIR, check=True)
        commit_msg = "feat(seo): standardize all 11 articles with 3-word slugs, <=60 char titles, semantic keyword prose & universal interlinking (1 home, 2 internal, 1 external)"
